...
router.get('/success',function(req,res,next){
    ...
    var txn_time=Date()
    var data={'txn_id':txn_id,'item_id':d.item_id,'item_name':d.item_name,'item_price':d.item_price,'txn_time':txn_time}
    usersmodel.paymentreq('payment',data,function(result){
    res.render('success')    
    })
...
router.all('/addpost',function(req,res,next){
   if(req.method=='GET')
   {
       res.render('addpost',{'result':'','catlist':d3,'subcatlist':d2})
   }
   else
   {
       var data=req.body
       ...
       var data1={'pid':pid,'title':data.title,'catnm':data.catnm,'subcatnm':data.subcatnm,'description':data.description,'price':data.price,'image1':f1,'image2':f2,'image3':f3,'mob':data.mob,'email':data.email,'address':data.address,'city':data.city,'ustatus':'0','vstatus':'0','postdate':postdate}
       usersmodel.addpost(data1,function(result){
           if(result)
               res.render('addpost',{'result':'Post added successfully please wait for verification','catlist':d3,'subcatlist':d2})
           else
               res.render('addpost',{'result':'post not added','catlist':d3,'subcatlist':d2})
       })
   }
});
